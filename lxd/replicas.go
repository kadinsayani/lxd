...
func replicaValidateConfig(config map[string]string) error {
	replicaConfigKeys := map[string]func(value string) error{
		...
		// lxdmeta:generate(entities=replica; group=conf; key=target_cluster)
		//
		// ---
		//  type: string
		//  shortdesc: Replica target cluster.
		//  scope: global
		"target_cluster": validate.Optional(validate.IsAny),
	}
...
//
//	Rename or run the replica
//
//	Renames or runs the replica.
//
//	---
//	consumes:
//	  - application/json
...
func replicaPost(d *Daemon, r *http.Request) response.Response {
	...
	// 	// Clear name from request if it matches the current name.
	// 	req.Name = ""
	// }
	// // Rename the replica if a new name is provided.
	// if req.Name != "" {
	// 	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
	// 		err = dbCluster.RenameReplica(ctx, tx.Tx(), name, req.Name)
	//
	// 		return err
	// 	})
	// 	if err != nil {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		dbClusterLink, err = dbCluster.GetClusterLink(ctx, tx.Tx(), replica.Config["target_cluster"])
		if err != nil {
			return fmt.Errorf("Failed to fetch cluster link %q: %w", replica.Config["target_cluster"], err)
		}
	...
	logger.Debug("Using target project", logger.Ctx{"projectName": replica.Project})
	...
	op, err := operations.OperationCreate(r.Context(), s, replica.Project, operations.OperationClassTask, operationtype.ReplicaJob, resources, nil, run, nil, nil)
	if err != nil {
		return response.InternalError(err)
	}
...
func replicasPost(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		var err error
		var dbClusterLink *dbCluster.ClusterLink
		dbClusterLink, err = dbCluster.GetClusterLink(ctx, tx.Tx(), req.Config["target_cluster"])
		if err != nil {
			return fmt.Errorf("Failed to fetch cluster link %q: %w", req.Config["target_cluster"], err)
		}
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		clusterLinkID, err := dbCluster.GetClusterLinkID(ctx, tx.Tx(), req.Config["target_cluster"])
		if err != nil {
			return err
		}
	...
	// TODO: Create replica on target cluster for restoring instances.
