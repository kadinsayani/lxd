...
SELECT services.id, services.identity_id, services.name, services.addresses, services.type, services.delegated, coalesce(services.description, '')
  FROM services
...
INSERT INTO services (identity_id, name, addresses, type, delegated, description)
  VALUES (?, ?, ?, ?, ?, ?)
`)
...
UPDATE services
  SET identity_id = ?, name = ?, addresses = ?, type = ?, delegated = ?, description = ?
 WHERE id = ?
`)
...
func serviceColumns() string {
	return "services.id, services.identity_id, services.name, services.addresses, services.type, services.delegated, coalesce(services.description, '')"
}
...
func UpdateService(ctx context.Context, tx *sql.Tx, name string, object Service) error {
	...
	result, err := stmt.Exec(object.IdentityID, object.Name, object.Addresses, object.Type, object.Delegated, object.Description, id)
	if err != nil {
