...
SELECT replicas.id, replicas.cluster_link_id, replicas.project_id, replicas.name, coalesce(replicas.description, '')
  FROM replicas
...
INSERT INTO replicas (cluster_link_id, project_id, name, description)
  VALUES (?, ?, ?, ?)
`)
...
UPDATE replicas
  SET cluster_link_id = ?, project_id = ?, name = ?, description = ?
 WHERE id = ?
`)
...
func replicaColumns() string {
	return "replicas.id, replicas.cluster_link_id, replicas.project_id, replicas.name, coalesce(replicas.description, '')"
}
...
func getReplicas(ctx context.Context, stmt *sql.Stmt, args ...any) ([]Replica, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		r := Replica{}
		err := scan(&r.ID, &r.ClusterLinkID, &r.ProjectID, &r.Name, &r.Description)
		if err != nil {
			return err
		}
...
func UpdateReplica(ctx context.Context, tx *sql.Tx, name string, object Replica) error {
	...
	result, err := stmt.Exec(object.ClusterLinkID, object.ProjectID, object.Name, object.Description, id)
	if err != nil {
