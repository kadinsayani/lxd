		...
		if !recursion {
			...
			for projectName, groups := range placementGroupNames {
				for _, placementGroup := range groups {
					u := entity.PlacementGroupURL(projectName, placementGroup)
...
func doPlacementGroupDelete(ctx context.Context, s *state.State, name string, projectName string) error {
	err := s.DB.Cluster.Transaction(ctx, func(ctx context.Context, tx *db.ClusterTx) error {
		...
		usedBy, err := cluster.GetPlacementGroupUsedBy(ctx, tx.Tx(), projectName, name)
		if err != nil {
			return err
		}
...
func placementGroupValidateConfig(config map[string]string) error {
	...
	// Policy is required
	policy, ok := config["policy"]
	if !ok || policy == "" {
		return api.StatusErrorf(http.StatusBadRequest, "Missing required config key: %q", "policy")
	}

	// Rigor is required
	rigor, ok := config["rigor"]
	if !ok || rigor == "" {
		return api.StatusErrorf(http.StatusBadRequest, "Missing required config key: %q", "rigor")
	}
