...
func Accept(state *state.State, gateway *Gateway, name, address string, schema, api, arch int) ([]db.RaftNode, error) {
	...
	err = state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		if !canPromote {
			...
			if err != nil {
				return fmt.Errorf("Failed getting cluster member %q roles: %w", address, err)
			}
...
// adjustRoles determines the next role change using the same generic roles
// algorithm as [app.RolesChanges.Adjust], while applying LXD-specific control-plane restrictions.
