...
func storagePoolVolumeDelete(d *Daemon, r *http.Request) response.Response {
	...
	err = doStoragePoolVolumeDelete(r.Context(), s, target, details.volumeName, details.volumeType, details.pool, effectiveProjectName, requestProjectName)
	if err != nil {
		return response.SmartError(err)
	}
...
func doStoragePoolVolumeDelete(ctx context.Context, s *state.State, target string, name string, volType cluster.StoragePoolVolumeType, pool storagePools.Pool, effectiveProjectName string, requestProjectName string) (err error) {
	...
	if len(volumeUsedBy) > 0 {
		if len(volumeUsedBy) != 1 || volType != cluster.StoragePoolVolumeTypeImage || !isImageURL(volumeUsedBy[0], dbVolume.Name) {
			return api.StatusErrorf(http.StatusBadRequest, "The storage volume is still in use")
		}
	}
