...
func imageDelete(d *Daemon, r *http.Request) response.Response {
	...
	op, err := doImageDelete(r.Context(), s, details.image.Fingerprint, details.imageID, projectName, isClusterNotification(r))
	if err != nil {
		return response.SmartError(err)
	}
...
func doImageDelete(ctx context.Context, s *state.State, fingerprint string, imageID int, projectName string, isClusterNotification bool) (*operations.Operation, error) {
	do := func(op *operations.Operation) error {
		...
		s.Events.SendLifecycle(projectName, lifecycle.ImageDeleted.Event(fingerprint, projectName, op.Requestor(), nil))
	...
	if err != nil {
		return nil, api.StatusErrorf(http.StatusInternalServerError, "Failed to create operation: %w", err)
	}
