...
func networkACLDelete(d *Daemon, r *http.Request) response.Response {
	...
	err = doNetworkACLDelete(r.Context(), s, projectName, aclName)
	if err != nil {
		return response.SmartError(err)
	}
...
func doNetworkACLDelete(ctx context.Context, s *state.State, projectName string, aclName string) error {
	netACL, err := acl.LoadByName(s, projectName, aclName)
	if err != nil {
		return err
	}
